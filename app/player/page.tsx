...
import { getVideoFile } from "@/lib/file-system"
...
export default function PlayerPage() {
    ...
    try {
      ...
      if (arcData) {
        ...
        if (episodesData[episodeIndex]) {
          ...
          await loadEpisodeVideo(episodesData[episodeIndex])
        }
      }
    } catch (error) {
  ...
  async function loadEpisodeVideo(episode: EpisodeMetadata) {
    try {
      ...
      const arcHandle = await rootHandle.handle.getDirectoryHandle(arc!.name)
      const fileHandle = await arcHandle.getFileHandle(episode.fileName)
